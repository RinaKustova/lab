var keyconfig_options = {
 ...
 getini: function(key) {
  var prefService = Components.classes["@mozilla.org/preferences-service;1"]
                                     ...
                                     .getBranch("extensions.vk_player_ext_panel.");
  try {
 ...
 store_old: function() {
  ...
  if (key!="") key=mod+key;
  var ikey=keyconfig_data.keys[oldkey];
 ...
 curvalue: function(key) {
  ...
  return keyconfig_options.getini(key);
 },

 /** Called on changing selection in treeview */
 select: function(tr) {
  var i=tr.currentIndex;
  keyconfig_options.store_old();
  var k=keyconfig_data.keys[i];
  var v=keyconfig_options.curvalue(k);
 ...
 /** Treeview model */
 treeView : {
  rowCount : keyconfig_data.keys.length,
  getCellText : function(row,column){
   var k=keyconfig_data.keys[row]
    if (column.id=="action") return keyconfig_options.actionName(k);
    var v=keyconfig_options.curvalue(k);
    return v;
  },
 ...
 initialize: function() {
  ...
  document.getElementById('keylist').view = keyconfig_options.treeView;
 },
 ...
 saveOptions: function() {
  ...
  keyconfig_options.store_old();
  for (var i=0;i<keyconfig_data.keys.length;i++) {
   var k=keyconfig_data.keys[i];
   var v=this.set_bindings[k];
   if (v==null) continue;
   try {
    keyconfig_options.setini(k,v);
   } catch (e) {
