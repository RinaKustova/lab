...
repeat: function(){
	vk_player_ext.repeat();
},
shuffle: function(){
	vk_player_ext.shuffle();
},
"return": function(){
	var f = vk_player_ext["return"];
	f();
},
...
//   alert("Unknown keycode:"+key);
  ...
  onKeyPress: function(evt) {
    ...
    //if (keyconfig.inputFocus()) return;
    ...
    if (evt.keyCode) {
     key=mod+keyconfig.tr(evt.keyCode);
    } else {
     if (key==32) key="Space";
    ...
    var kb=keyconfig.bindings[key];
    if (kb==undefined) return;
    if (!kb) return;
    keyconfig[kb]();
    keyconfig.stopEvent(evt);
  },
